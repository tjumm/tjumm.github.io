...
<div class="col-md-8 clearfix">
  ...
  <p>Jie Wang, Guang Li, Zhijian Huang, Chenxu Dang, Hangjun Ye, Yahong Han, Long Chen<br>CVPR 2026,<br>Cross-view 3D geometric modeling capability is crucial for autonomous driving, but existing Vision-Language Models (VLMs) lack this capability, leading to poor performance. Current auxiliary training methods with Q&A data fail to solve the problem fundamentally. Thus, we propose VGGDrive, an architecture that equips VLMs with cross-view geometric grounding from mature 3D foundation models via a plug-and-play Cross-View 3D Geometric Enabler (CVGE) to bridge 3D and 2D features. Extensive experiments show VGGDrive improves VLM performance on five autonomous driving benchmarks. We believe integrating mature 3D foundation models can empower autonomous driving and hope this exploration demonstrates the paradigm’s potential.
</p>
...
<div class="col-md-8 clearfix">
  ...
  <p>Zihao Zhang, Yang Li, Aming Wu, Yahong Han, Jialie Shen<br>CVPR 2026<a href='http://arxiv.org/abs/2510.13307'>(Preprint)</a>, <a href='https://github.com/Brucely674/Causal-3D-NCDSS'>(Project Page)</a> <br>Novel Class Discovery in Point Cloud Segmentation has recently emerged as an important research direction, aiming to leverage knowledge learned from known categories to automatically discover and segment previously unseen classes in unlabeled point clouds. The fundamental challenge of this task lies in effectively transferring both geometric and semantic knowledge from multiple known classes to facilitate reliable semantic understanding and accurate segmentation of novel categories.
However, existing approaches largely overlook the high-order structural relationships between known and novel classes. Most methods rely primarily on pairwise (binary) associations for class assignment and reasoning, which restricts the model to local similarity matching and limits its ability to capture complex inter-class dependencies. As a result, the semantic representation of novel classes often becomes ambiguous, leading to suboptimal segmentation performance.
To overcome these limitations, we introduce a hypergraph-based modeling framework that explicitly captures high-order associations among multiple classes. By establishing collaborative relational structures across known categories, the proposed framework enables collective reasoning toward novel classes, moving beyond conventional binary relation modeling. Furthermore, current methods tend to emphasize semantic feature extraction while insufficiently exploiting geometric characteristics inherent in point clouds. In contrast, our approach jointly considers geometric structure and semantic information, enabling more robust representation learning and improving the accuracy and stability of novel class segmentation.
